"""
app/models/schemas.py - Add these new models to your existing schemas
"""
...
class UserLimits(BaseModel):
    ...
    monthly_tokens: int = Field(default=1000000, ge=0)
...
class OpenAIKeyUpdate(BaseModel):
    """Request to set/update OpenAI API key"""
...
class OpenAIKeyStatus(BaseModel):
    """OpenAI key status response"""
    has_custom_key: bool
    key_type: str  # "custom" or "system"
    message: str
...
class UsageHistoryItem(BaseModel):
    """Individual usage history record"""
    timestamp: datetime
    action: str  # "message_sent", "assistant_created", "thread_created"
...
class AssistantUsageStats(BaseModel):
    """Usage statistics for a specific assistant"""
    assistant_id: str
    assistant_name: str
    total_messages: int
    total_tokens: int
...
class RateLimitError(BaseModel):
    ...
    code: str  # "DAILY_LIMIT", "WEEKLY_LIMIT", "TOKEN_LIMIT"
    message: str
    used: int
    limit: int
    resets_at: datetime
