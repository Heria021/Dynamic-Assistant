        ...
        formatted_items.append(
            {
                ...
                "used_custom_key": item.get("metadata", {}).get(
...
async def get_assistant_usage_stats(
    ...
    user_id = current_user.get("sub") or current_user.get("user_id")
    assistant = await db["assistants"].find_one({"_id": assistant_id, "userId": user_id})

    if not assistant:
        raise HTTPException(
            status_code=404, detail="Assistant not found or you don't have permission"
        )

    cursor = db["usage_history"].find(
        {"userId": user_id, "assistantId": assistant_id}
    )
    ...
    last_used = max(i.get("timestamp") for i in items if i.get("timestamp"))
...
async def export_usage_data(
    ...
    user_id = current_user.get("sub") or current_user.get("user_id")
    cursor = db["usage_history"].find({"userId": user_id}).sort("timestamp", -1)
